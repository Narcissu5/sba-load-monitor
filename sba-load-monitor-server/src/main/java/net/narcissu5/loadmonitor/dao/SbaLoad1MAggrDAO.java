...
public class SbaLoad1MAggrDAO {
    ...
    public List<Load1MAggr> sinceThen(int minute) {
        return jdbcTemplate.query("SELECT id,count,minute,app_name,s1xx,s2xx,s3xx,s4xx,s5xx,created_at " +
                "FROM load_1m_aggr WHERE minute > ? " +
                "ORDER BY app_name,minute", Load1MAggrRowMapper.INSTANCE, minute);
    }
}
